...
from sc_model.models.sc_model import sc_model_single_cell
...
parser = argparse.ArgumentParser(
    prog="sc_model",
    description="""
    Train and evaluate a single-cell spatial contrast model on the specified dataset and stimulus.
    
    The model consists of a linear filter, representing an estimate of the cell's receptive field, followed by a
    computation of the mean luminance and local spatial contrast signals within the receptive field. These signals
    are linearly combined before being passed through a non-linear activation function. The parameters of the model 
    are optimized using maximum likelihood estimation (MLE). The model is trained on the specified dataset and
    stimulus, and the results are saved to a different pickle file for each combination of input parameters.
    """,
)
parser.add_argument(
...
if __name__ == "__main__":
    ...
    output_path = (
        RESULTS_PATH / dataset / stimulus / f"seed_{stimulus_seed}" / "sc_model" / f"per_cell_{dataset}_"
    ...
    if output_path.exists() and not overwrite:
        ...
        output = sc_model_single_cell(
            ...
            sigma_window=3.0,
            mle_method="L-BFGS-B",
            starter_params=[running_resp.max(), -2.0, 1.0, 0.0],
        ...
        params = {
            ...
            "w1": fit_parameters[2],
            "w2": fit_parameters[3],
            "w": fit_parameters[3] / fit_parameters[2],
        }
